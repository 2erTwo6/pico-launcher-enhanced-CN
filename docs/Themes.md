...
- **topLaunchInfo** - Where the game's markers go: a crown on the most launched game, a check on a completed one and a heart on a favorite. Left out, they sit centred above `topIcon`, half over the card's top edge and with nothing under them, the way the Material theme does; one marker or three, the row stays centred on the icon. Set it to put them on a pill instead: `position` is the top-right corner of that pill, which grows to the left. Set `hidden` to `true` to remove them entirely. The launch count and play time that used to sit on the pill are switched off in this version.
