...
![Material 主题封面流：启动最多的游戏带金星，已收藏和已通关的带爱心与对勾，标记显示在图标上方](docs/images/enhanced/Coverflow.png)
![Material 主题图标网格：启动最多的游戏带金星，已收藏和已通关的带爱心与对勾，标记显示在图标上方](docs/images/enhanced/Grid.png)
![自定义主题：游戏标记位于图标上方，在任何背景图上都清晰可读](docs/images/enhanced/CustomTheme.png)
![收藏面板：跨文件夹显示所有收藏游戏及其游玩时间](docs/images/enhanced/Favorites.png)
![统计面板：总览、最常启动的游戏和最近游玩的游戏](docs/images/enhanced/Statistics.png)
![菜单：最近游玩、收藏、统计、删除游戏，以及两个筛选器及其状态](docs/images/enhanced/Menu.png)
![关于面板：LNH 团队的 Pico Launcher、rasalopa 的增强版、构建来源，以及操作速查表](docs/images/enhanced/About.png)
...
- **以菜单取代一排图标** —— 应用栏只保留返回和显示设置，并新增一个三点按钮；最近游玩、收藏、统计、删除和两个筛选器都收纳在它打开的菜单里，每一项都标注名称
- **关于面板** —— 从菜单的标题行打开：谁做了什么、确切的构建版本和来源仓库，以及没有专属按钮的操作速查表
- **按首字母跳转** —— 在按名称排序的文件夹中，按 L 或 R 跳到下一个首字母；落点字母会在触摸屏底部短暂显示
- **游戏数量显示** —— 当前文件夹的游戏数量，显示在统计面板顶部
- **随机启动游戏** —— SELECT + A
- **收藏** —— 在游戏上按 X 收藏，顶屏会显示爱心标记
- **通关标记** —— 长按 X 标记为已通关，顶屏会显示绿色对勾
- **收藏与通关筛选** —— 菜单中的"仅收藏"和"仅通关"选项，启用时会显示状态
- **收藏面板** —— 从菜单打开，查看所有文件夹中的收藏游戏；点击条目即可跳转
- **最近游玩面板** —— 从菜单打开；点击条目即可跳转到对应游戏
- **统计面板** —— 从菜单打开，查看总数、最常启动的游戏和启动器版本
- **截图** —— 长按 START 约半秒，将上下屏幕保存到 `/_pico/screenshots`
- **单游戏启动记录** —— 每个游戏独立记录启动次数和最近游玩日期；最近游玩面板显示各游戏的日期，统计面板显示启动次数最多的三个游戏
- **大致游玩时长** —— 按游戏显示，见收藏面板
- **删除游戏** —— 从菜单打开，确认后删除 ROM 及其存档
- **亮度控制** —— 在显示设置中调节 DS Lite 背光亮度
- **隐藏空文件夹** —— 可选；没有可玩内容的文件夹不会显示在列表中
- **更好用的金手指列表** —— 列表首尾可循环，按 X 可一键关闭全部金手指
- **顶屏游戏标记** —— 启动最多的游戏显示金星，收藏显示爱心，通关显示对勾，均位于游戏图标上方；[自定义主题](docs/Themes.md)可以移动或隐藏它们
